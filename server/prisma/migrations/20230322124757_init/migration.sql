/*
  ...
  - A unique constraint covering the columns `[id,userId]` on the table `habits` will be added. If there are existing duplicate values, this will fail.
...
CREATE TABLE "new_day_habits" (
    ...
    CONSTRAINT "day_habits_habit_id_userId_fkey" FOREIGN KEY ("habit_id", "userId") REFERENCES "habits" ("id", "userId") ON DELETE RESTRICT ON UPDATE CASCADE,
    CONSTRAINT "day_habits_userId_fkey" FOREIGN KEY ("userId") REFERENCES "users" ("id") ON DELETE RESTRICT ON UPDATE CASCADE
);
INSERT INTO "new_day_habits" ("day_id", "habit_id", "id", "userId") SELECT "day_id", "habit_id", "id", "userId" FROM "day_habits";
